...
async function flush() {
  ...
  if (chainId == 1) {
    ...
    const rainbiwWallProtocolContractAddress =
      "0x1C0d02F86Dd9F46833c7558607166656d97CF98d";
...
function getCard() {
  return $(
    '<div class="card shadow mb-4 "><div class="card-header price-bar" style="display:none;"></div><div class="card-body"><p class="text-rainbow text-center"></p></div><div class="card-footer operate-bar" style="display:none;"><span style="margin-left:0">BUY</span><span style="margin-right:0">SELL</span></div></div>'
  );
}
...
async function appendContent(index) {
  let card = getCard();
  let textBody = card.find(".text-rainbow");
  let priceBar=card.find(".price-bar")
  priceBar.html("PRICE : 0.01 ETH")
  let cid = await rainbowWallProtocol.methods.tokenURI(index + 1).call();
  let token = await rainbowWallProtocol.methods.tokenList(index).call();
  ...
  $(".card").hover(function () {
    $(this).children(".price-bar").toggle();
    $(this).children(".operate-bar").toggle();
  });
}
