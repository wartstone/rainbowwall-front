...
// const ipfs = window.IpfsHttpClient.create({ host: 'http://localhost/', port: "5001" })
...
async function flush() {
  ...
  if (chainId == 1) {
    ...
    rainbiwWallProtocolContractAddress =
      "0x93CEA32ee5bafC38CAC334f5258F1d7df6172419";
...
async function showSymbolData(tokenId) {
  ...
  let balance = await rainbowWallToken.methods
    ...
    .call();
  $(".token-balance").html(
    "Balance: " + toFixedFloor(balance / 1e18, 6) + " " + symbol
  );
...
async function getSwapBarData() {
  ...
  $(".token-amount").keyup(async function () {
    ...
    if (tokenId) {
      ...
      $(".eth-amount").val(toFixedFloor((ethAmount * 0.99) / 1e18, 6));
  ...
  $(".eth-amount").keyup(async function () {
    ...
    if (tokenId) {
      ...
      let amount = await rainbowWallToken.methods
        ...
        .call();
      console.log(amount);
      $(".token-amount").val(toFixedFloor((amount * 0.99) / 1e18, 6));
      $(".swap").attr("disabled", false);
      $(".swap").html('<span class="text-rainbow">SWAP</span>');
    }
  });
...
async function paste() {
  ...
  console.log("pre constructMetadata");

  let metadata = constructMetadata(content, "", TYPE.TEXT);

  console.log("aft constructMetadata");

  let cid = await uploadToIpfs(metadata);

  console.log(`cid = ${cid}, metadata = ${metadata}`);
