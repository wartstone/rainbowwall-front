...
async function flush() {
  ...
  if (chainId == 1) {
    ...
    uint="ETH"
    const rainbiwWallProtocolContractAddress =
...
function getCard() {
  return $(
    '<div class="card shadow mb-4 "><div class="card-header price-bar" ></div><div class="card-body"><p class="text-rainbow text-center"></p></div><div class="card-footer operate-bar" ><div class="row"><button class="col buy-bar btn" onclick="buy(1)"> BUY </button> <button class="col sell-bar btn"> SELL </button> </div></div></div>'
  );
}
...
async function appendContent(tokenId) {
  ...
  let buyBar = card.find(".buy-bar");
  let sellBar = card.find(".sell-bar");
  ...
  buyBar.attr("tokenAddress", tokenAddress);

  sellBar.attr("tokenAddress", tokenAddress);
  ...
  priceBar.html(symbol + " - PRICE : " + price / 1e18 + " "+uint);
