...
const RainbowWallProtocolABI = [
  ...
  },
  {
    ...
    outputs: [
      {
        internalType: "contract RainbowWallToken",
        name: "",
        type: "address",
      },
    ],
...
async function flush() {
  ...
  if (chainId == 1) {
    ...
    rainbiwWallProtocolContractAddress =
      "0xdab31242325B282ec101CfD595CF54ABe5E10aE2";
    rainbowWallProtocol = new web3.eth.Contract(
      RainbowWallProtocolABI,
      rainbiwWallProtocolContractAddress
    );
...
async function showSymbolData(tokenId) {
  $(".contentView").find(".card").remove();
  $(".token-balance").html("");
  $(".swap-bar").remove("tokenId");
  console.log(tokenId);
  let tokenAddress = await rainbowWallProtocol.methods
    .tokenList(tokenId)
    .call();
  appendContent(tokenId);
  let rainbowWallToken = new web3.eth.Contract(
    RainbowWallTokenABI,
    tokenAddress
  );
  let symbol = await rainbowWallToken.methods.symbol().call();
  $(".selecToken").html(symbol);
  let balance = await rainbowWallToken.methods
    .balanceOf(selectedAccount)
    .call();
  $(".token-balance").html(
    "Balance: " + (balance / 1e18).toFixed(6) + " " + symbol
  );
  $(".swap-bar").attr("tokenId", tokenId);
}

async function getSwapBarData() {
  let ethBalance = await web3.eth.getBalance(selectedAccount);
  $(".uint-balance").html(
    "Balance: " + (ethBalance / 1e18).toFixed(6) + " " + uint
  );
  $(".token-input").keyup(async function () {
    $(".symbol").html("");
    ...
    $(".symbol").html(symbol);
    $(".symbol").attr("onclick", "showSymbolData(" + tokenId + ")");
  });
  $(".changePosition").click(function () {
    let removeDiv1 = $(".eth-form");
    let removeDiv2 = $(".token-form");
    ...
    $(".swap").html("Enter an amount");
    $(".eth-amount").val(0);
    $(".token-amount").val(0);
    if ($(".eth-amount").attr("disabled")) {
      $(".eth-amount").attr("disabled", false);
      $(".token-amount").attr("disabled", true);
    } else {
      $(".token-amount").attr("disabled", false);
      $(".eth-amount").attr("disabled", true);
    }
  });
  $(".token-amount").keyup(async function () {
    $(".swap").attr("disabled", true);
    $(".swap").html("Enter an amount");
    let amount = $(this).val();
    ...
    if (tokenId) {
      ...
      let ethAmount = await rainbowWallToken.methods
        .getAmountIn((amount * 1e18).toString())
        .call();
      console.log(ethAmount);
      $(".eth-amount").val(((ethAmount * 0.99) / 1e18).toFixed(6));
      $(".swap").attr("disabled", false);
      $(".swap").html('<span class="text-rainbow">SWAP</span>');
    }
  });
  $(".eth-amount").keyup(async function () {
    $(".swap").attr("disabled", true);
    $(".swap").html("Enter an amount");
    let ethAmount = $(this).val();
    let tokenId = $(".swap-bar").attr("tokenId");
    if (tokenId) {
      let tokenAddress = await rainbowWallProtocol.methods
        .tokenList(tokenId)
      ...
      let amount = await rainbowWallToken.methods
        .getAmountOut((ethAmount * 1e18).toString())
        .call();
      console.log(amount);
      $(".token-amount").val(((amount * 0.99) / 1e18).toFixed(6));
      $(".swap").attr("disabled", false);
      $(".swap").html('<span class="text-rainbow">SWAP</span>');
    }
  });
...
async function buy(amount) {
  let tokenAddress = $(this).attr("tokenAddress");
  let rainbowWallToken = new web3.eth.Contract(
    RainbowWallTokenABI,
    tokenAddress
  );
  rainbowWallToken.methods.buy(selectedAccount).send({ value: amount });
}
...
async function sell(amount) {
  ...
  rainbowWallToken.methods.sell(selectedAccount, amount);
}
...
async function appendContent(tokenId) {
  ...
  priceBar.html(
    symbol + " - TOKEN PRICE : " + (price / 1e18).toFixed(6) + " " + uint
  );
